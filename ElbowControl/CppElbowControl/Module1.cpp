...
CModule1::CModule1()
	...
	, m_counter(0)
	, PID4Cylinder(1.6, 0.005, 0.125)//PID参数在这里改
	, Pi(3.14159f)
{
...
HRESULT CModule1::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (timer > 2)
	{
		...
		//float relative = PID4Elbow.pid_control(SetAngle, elbow.ShowAngle());
...
{
	////电压转换为（0-10V）->（0-10000）
	//m_Outputs.m1Int = static_cast<int>(elbow.ShowM1() / 10 * 10000);
	//m_Outputs.m2Int = m_Outputs.m3Int = static_cast<int>(elbow.ShowM2() / 10 * 10000);
