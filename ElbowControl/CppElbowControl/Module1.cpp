...
HRESULT CModule1::CycleUpdate(ITcTask* ipTask, ITcUnknown* ipCaller, ULONG_PTR context)
{
	...
	if (timer > 2)
	{
		...
		//SetRad = (10.0 / 180.0 * Pi);
		SetRad = static_cast<float>(0.4 * sin_(2 * Pi*0.1*(timer - 2) + 1.5*Pi) + 0.18);
...
{
	elbow.GetAngle(m_Inputs.Angle - 270);//不受力状态时，角度为270，减去
